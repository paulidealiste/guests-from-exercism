defmodule LinkedList do
  ...
  def counter({_, {}}, acc), do: acc + 1
  def counter({_, next}, acc), do: counter(next, acc + 1)
  ...
  def lister({data, {}}, acc), do: [data | acc] |> Enum.reverse()
  def lister({data, next}, acc), do: lister(next, [data | acc])
  ...
  def rvr({data, {}}, ll), do: ll |> push(data)
  def rvr({data, next}, ll), do: rvr(next, ll |> push(data))
end
