defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
         ...
         {:ok, checksum} <- stamp_form(timestamp, counter.(birthday), form) do
      {:ok, get_new_passport_number(timestamp, counter.(birthday), checksum)}
    else
      {:coffee_break, _instructions} ->
        {:retry,
         DateTime.add(DateTime.from_naive!(now, "Etc/UTC"), 15, :minute) |> DateTime.to_naive()}
