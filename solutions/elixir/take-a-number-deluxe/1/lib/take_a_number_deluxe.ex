defmodule TakeANumberDeluxe do
  ...
  # Server callbacks
  @impl GenServer
  def init(min: min, max: max, timeout: timeout) do
    state = TakeANumberDeluxe.State.new(min, max, timeout)
  ...
  def handle_call(:queue_new_number, _from, state) do
    queued = TakeANumberDeluxe.State.queue_new_number(state)
  ...
  def handle_call({:serve_next_queued_number, priority_number}, _from, state) do
    nextq = TakeANumberDeluxe.State.serve_next_queued_number(state, priority_number)
    timeout = state.auto_shutdown_timeout
  ...
  @impl GenServer
  def handle_cast(:reset_state, %TakeANumberDeluxe.State{
        min_number: min,
        max_number: max,
